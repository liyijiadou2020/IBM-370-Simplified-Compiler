...
int SDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  ...
  if
    ...
    )                                             /* то                     */
   {
    ...
    temp = atoi ( RAB );                 /*перевод ASCII-> int     */
	RAB = (char *) &temp;                /*приведение к соглашениям*/ 
	memcpy ( RX.BUF_OP_RX + 3 , RAB,  1);          /* ЕС ЭВМ                 */
	memcpy ( RX.BUF_OP_RX + 2 , RAB + 1 ,  1);
	memcpy ( RX.BUF_OP_RX + 1 , RAB + 2,  1);
	memcpy ( RX.BUF_OP_RX , RAB + 3 ,  1);
   }
