 ...
 struct ASSKARTA                                  /*структ.карты АССЕМБЛЕРА */
  {
   ...
   unsigned  char OPERAND  [20];                  /*поле операнда           */
   unsigned  char PROBEL3  [ 1];                  /*пробел разделитель      */
   unsigned  char COMM     [44];                  /*поле комментария        */
  };
...
*/
  ...
  struct OPRX                                     /*структ.буф.опер.форм.RX */
   {
    ...
    short int B2D2;                                     /*X2 - второй операнд     */
  ...
  {
	  unsigned char OP;
	  unsigned char R1ZZ; /* ZZ - не используется */
	  short int B2D2;
  };
...
void STXT( int type )                              /*подпр.формир.TXT-карты  */
 {
  ...
  case TRR:
		...
		oplength = sizeof(RR.BUF_OP_RR);
		memcpy ( TXT.STR_TXT.OPER, RR.BUF_OP_RR , oplength ); /* для RR-формата         */
	  break;
  case TRX:
	    oplength = sizeof(RX.BUF_OP_RX);
		memcpy ( TXT.STR_TXT.OPER , RX.BUF_OP_RX , oplength );/* для RX-формата         */
	  break;
  case TRS:
	    oplength = sizeof(RS.BUF_OP_RS);
		memcpy ( TXT.STR_TXT.OPER , RS.BUF_OP_RS , oplength );/* для RS-формата         */
	  break;
  case TDC:
	    oplength = sizeof(DCDSWORD.BUF_WORD);
		memcpy ( TXT.STR_TXT.OPER , DCDSWORD.BUF_WORD , oplength );
	  break;
  case TDS:
	    oplength = sizeof(DCDSWORD.BUF_WORD);
	    memcpy ( TXT.STR_TXT.OPER , DCDSWORD.BUF_WORD , oplength );	
  ...
  CHADR = CHADR + oplength;                            /*коррекц.счетчика адреса */
  return;
 }

void INVERT(char* ptr, int nbytes)
{
	for(int i = 0; i < nbytes-1; i++)
	{
		char* cptr = ptr;
		for(int j = 0; j < nbytes-1-i; j++)
		{
			swab(cptr,cptr,2);
			cptr++;
		}
...
int SDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  ...
  //RX.OP_RX.R1X2 = 0;                              /*байта RX.OP_RX          */
   if(!memcmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "F'", 2))												
   {
	...
	DCDSWORD.WORD = atoi(RAB);
	INVERT((char*)&DCDSWORD.WORD,4);
   }
   else if(!memcmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "BL4'", 4))
   {
	RAB=strtok                                    /*в перем. c указат.RAB   */
	 (                                        /*выбираем первую лексему */
	...
	DCDSWORD.WORD = BSTOMEMORY(RAB);
   }
   else return (1);                                    /*сообщение об ошибке     */

   STXT (TDC);                                       /*формирование TXT-карты  */
...
int SDS()                                         /*подпр.обр.пс.опер.DS    */
 {
  if
	...
	DCDSWORD.WORD = 0;                             /*занулим RX.OP_RX.B2D2   */
  else                                            /*иначе                   */
   return (1);                                    /*сообщение об ошибке     */

  STXT (TDS);                                       /*формирование TXT-карты  */
...
int SRX()                                         /*подпр.обр.опер.RX-форм. */
 {
  ...
  OPERAND1 = strtok                                 /*в перем. c указат.METKA1*/
	   (                                      /*выбираем первую лексему */
    (char*) TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, /*операнда текущей карты  */
	    ","                                   /*исх.текста АССЕМБЛЕРА   */
	   );

  OPERAND2 = strtok                                 /*в перем. c указат.METKA2*/
	   (                                      /*выбираем вторую лексему */
	    NULL,                                 /*операнда текущей карты  */
	    " "                                   /*исх.текста АССЕМБЛЕРА   */
	   );

  if ( isalpha ( (int) *OPERAND1 ) )                /*если лексема начинается */
   {                                              /*с буквы, то:            */
    for ( J=0; J<=ITSYM; J++ )                    /* все метки исх.текста в */
     {                                            /* табл. T_SYM сравниваем */
													/* со знач.перем. *METKA  */
      METKA = strtok ((char*) T_SYM[J].IMSYM , " ");
      if( !strcmp ( METKA , OPERAND1 ) )            /* и при совпадении:      */
       {                                          /*  берем значение этой   */
   ...
   {                                              /*перв.операнда машинн.ком*/
     R1X2 = atoi ( OPERAND1 ) << 4;                 /*значен.выбр.   лексемы  */
   }
 ...
 SRX1:
  ...
  if ( isalpha ( (int) *OPERAND2 ) )                /*если лексема начинается */
   {                                              /*с буквы, то:            */
    for ( J=0; J<=ITSYM; J++ )                    /* все метки исх.текста в */
     {                                            /* табл. T_SYM сравниваем */
						  /* со знач.перем. *МЕТКА  */
      METKA = strtok ((char*) T_SYM[J].IMSYM , " ");
      if( !strcmp ( METKA , OPERAND2 ) )					/* и при совпадении:      */
       {												/*  установить нач.знач.: */
  ...
  else if(isdigit ( (int) *OPERAND2 ))
  {
	  D2 = strtok (OPERAND2 , "(");
	  X2 = strtok (NULL , ",");
	  B2 = strtok (NULL , ")");
	    /* если в X2 буква, то подставляем значение номера регистра */
	  if ( isalpha ( (int) *X2 ) )
	  {
		  for ( J=0; J<=ITSYM; J++ )
		  {
...
SRX3: 
	  /* если в B2 буква, то подставляем значение номера регистра */
	  if ( isalpha ( (int) *B2 ) )
	  {
		  for ( J=0; J<=ITSYM; J++ )
		  {
...
SRX4:
 ...
 SRX2:
  ...
  STXT(TRX);                                        /*формирование TXT-карты  */
  return(0);                                      /*выйти из подпрограммы   */
 }
...
SRS2:
 ...
 SRX2:
  ...
  RS.OP_RS.R1ZZ = R1X2;                           /*дозапись перв.операнда  */
  B2D2 = B2D2 + DELTA;                   
  PTR = (char *)&B2D2;
  swab ( PTR , PTR , 2 );
  RS.OP_RS.B2D2 = B2D2;

  STXT(TRS);                                        /*формирование TXT-карты  */
  return(0);                                      /*выйти из подпрограммы   */
 }
/*..........................................................................*/
...
*/
  ...
  for ( I1=0; I1 < NASSTEXT; I1++ )             /*для карт с 1 по конечную*/
   {                                              /*                        */
